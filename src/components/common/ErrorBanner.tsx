...
export const ErrorBanner: React.FC<ErrorBannerProps> = ({
  ...
  return (
    <div className="bg-red-50/90 border border-vermilion/30 rounded-2xl p-5 shadow-warm-sm max-w-xl mx-auto my-6 text-charcoal">
      <div className="flex items-start space-x-3.5">
        <AlertCircle className="w-5 h-5 text-vermilion shrink-0 mt-0.5" />
        <div className="flex-1">
          <h4 className="text-sm font-bold text-vermilion-deep">{title}</h4>
          <p className="text-xs text-charcoal-muted mt-1 leading-relaxed">{message}</p>
          {onRetry && (
            <button
              onClick={onRetry}
